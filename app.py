...
hotel_review= pd.read_pickle('./data/final.pkl')
...
def result():
    if request.method == 'POST':
        if request.form['options']=='reviews':
            ...
            return get_hotel(mydict=new_recommendations(var_hotel,var_city,cosine_similarities=cosine),city=var_city)
        elif request.form['options']=='tags':
            var_city=request.form['city']
            var_hotel=request.form['hotel']
            return get_hotel(mydict=new_recommendations_tags(var_hotel,var_city,cosine_similarities=similarity),city=var_city)
    else:
        return render_template('new.html')
